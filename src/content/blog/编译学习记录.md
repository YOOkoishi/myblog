...
符号表借鉴ai的写法增添了很多 rubustness 。 现在可以很方便的用 `enterScope()` 进入新作用域 `exitScope()` 退出作用域。
...
现在符号表搞好了，还需要干一些事情，就是在 `visitConstDef()` 的时候，要把常量折叠了。 再存进符号表里面， 等后面用到LVal 的时候再把这个加入到 IR 中 block 的 value 列表里面。
