...
我貌似发现了一些问题，我的Bison文件中，括号里的Exp好像并没有被优先处理。
...
还有一些其他的Parser规则,比如 LL 以及其他,这里用ai做了个表.
...
不过我们等会再说这两个table是怎么生成,我们先看看LR Parser 会进行哪些操作.
...
这个时候我们可以得到一条 ACTION 表的语句 当我们在状态 I1 , 然后读取到了 INT_CONST 的时候,我们把这个INT_CONST 压入 到符号栈栈顶 同时把状态 I2 压入状态栈的栈顶
...
假设上面的这个状态是 I7 ,那么我们就得到一个goto表的项了
